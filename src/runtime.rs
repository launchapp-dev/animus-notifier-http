...
//!
//! Lifted from the legacy in-tree `orchestrator-notifications` crate. The
//! shape of the on-disk state (`~/.animus/<repo-scope>/notifications/`)
//! and the configuration schema (`animus.daemon-notification-config.v1`,
//! stored under `notification_config` in `pm-config.json`) are
//! intentionally unchanged so an upgrade from the in-tree implementation
//! to this plugin transparently adopts existing state.

use animus_notifier_protocol::DaemonEventRecord;
...
const NOTIFIER_USER_AGENT: &str = "animus-notifier-http/1";
...
impl DaemonNotificationRuntime {
    pub fn new(project_root: &str) -> Result<Self> {
        ...
        let client = Client::builder()
            .user_agent(NOTIFIER_USER_AGENT.to_string())
            .build()
...
fn pm_config_path(project_root: &str) -> PathBuf {
    ...
    if let Some(scoped) = scoped_state_root(&path) {
...
fn repo_scope(project_root: &str) -> String {
    repository_scope_for_path(Path::new(project_root))
}
...
// -------------------------------------------------------------------
// Inlined `protocol::repository_scope` helpers.
//
// Lifted from `protocol::repository_scope` so the plugin avoids a
// runtime dependency on the in-tree `protocol` crate. Keeps the on-disk
// scope identical to the daemon's view so upgrading the in-tree
// notification runtime to this plugin reuses existing state under
// `~/.animus/<repo-scope>/notifications/`.
// -------------------------------------------------------------------
